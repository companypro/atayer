              ...
              if(isGuestLogIn && orderController.guestAddress == null && orderController.orderType != 'take_away') {
                ...
                if(widget.storeId == null){
                  ...
                  for (int index = 0; index < _cartList!.length; index++) {
                    ...
                    print("Campaign ID: ${cart.isCampaign! ? cart.item!.id : null}");
                    print("Discounted Price: ${cart.discountedPrice.toString()}");
                    print("Empty String Placeholder: ''"); // Assuming this is a placeholder as no actual value is provided.
                    print("Variation: ${Get.find<SplashController>().getModuleConfig(cart.item!.moduleType)!.newVariation! ? null : cart.variation}");
                    print("Variations: ${Get.find<SplashController>().getModuleConfig(cart.item!.moduleType)!.newVariation! ? variations : null}");
                    print("Quantity: ${cart.quantity}");
                    print("Add On ID List: ${addOnIdList}");
                    print("Add Ons: ${cart.addOns}");
                    print("Add On Quantity List: ${addOnQtyList}");
                    print("Item Type: 'Item'");
                    print("Item Type Detail: ${!widget.fromCart ? 'App\Models\ItemCampaign' : null}");
                  ...
                  PlaceOrderBody placeOrderBody = PlaceOrderBody(
                    ...
                    scheduleAt: !storeController.store!.scheduleOrder! ? null : (orderController.selectedDateSlot == 0 && orderController.selectedTimeSlot == 0) ? null : DateConverter.dateToDateAndTime(scheduleEndDate),
                    orderAmount: total,
                    orderNote: Get.find<OrderController>().noteController.text,
                    ...
                    // storeId: _cartList![i]!.item!.storeId,
                  ...
                  // print("Cart: ${carts}");
                  // print("Coupon Discount Amount: ${Get.find<CouponController>().discount}");
                  // print("Distance: ${orderController.distance}");
                  // print("Scheduled At: ${!storeController.store!.scheduleOrder! ? null : (orderController.selectedDateSlot == 0 && orderController.selectedTimeSlot == 0) ? null : DateConverter.dateToDateAndTime(scheduleEndDate)}");
                  // print("Order Amount: ${total}");
                  // print("Order Note: ${Get.find<OrderController>().noteController.text}");
                  // print("Order Type: ${orderController.orderType}");
                  // print("Payment Method: ${orderController.paymentMethodIndex == 0 ? 'cash_on_delivery' : orderController.paymentMethodIndex == 1 ? 'wallet' : orderController.paymentMethodIndex == 2 ? 'digital_payment' : 'offline_payment'}");
                  // print("Coupon Code: ${(Get.find<CouponController>().discount! > 0 || (Get.find<CouponController>().coupon != null && Get.find<CouponController>().freeDelivery)) ? Get.find<CouponController>().coupon!.code : null}");
                  // print("Store ID: ${_cartList![i]!.item!.storeId}");
                  // print("Address: ${finalAddress!.address}");
                  // print("Latitude: ${finalAddress.latitude}");
                  // print("Longitude: ${finalAddress.longitude}");
                  // print("Address Type: ${finalAddress.addressType}");
                  // print("Contact Person Name: ${finalAddress.contactPersonName ?? '${Get.find<UserController>().userInfoModel!.fName} ${Get.find<UserController>().userInfoModel!.lName}'}");
                  // print("Contact Person Number: ${finalAddress.contactPersonNumber ?? Get.find<UserController>().userInfoModel!.phone}");
                  // print("Street Number: ${isGuestLogIn ? finalAddress.streetNumber??'' : orderController.streetNumberController.text.trim()}");
                  // print("House: ${isGuestLogIn ? finalAddress.house??'' : orderController.houseController.text.trim()}");
                  // print("Floor: ${isGuestLogIn ? finalAddress.floor??'' : orderController.floorController.text.trim()}");
                  // print("Discount Amount: ${discount}");
                  // print("Tax Amount: ${tax}");
                  // print("DM Tips: ${(orderController.orderType == 'take_away' || orderController.tipController.text == 'not_now') ? '' : orderController.tipController.text.trim()}");
                  // print("Cutlery: ${Get.find<CartController>().addCutlery ? 1 : 0}");
                  // print("Unavailable Item Note: ${Get.find<CartController>().notAvailableIndex != -1 ? Get.find<CartController>().notAvailableList[Get.find<CartController>().notAvailableIndex] : ''}");
                  // print("Delivery Instruction: ${orderController.selectedInstruction != -1 ? AppConstants.deliveryInstructionList[orderController.selectedInstruction] : ''}");
                  // print("Partial Payment: ${orderController.isPartialPay ? 1 : 0}");
                  // print("Guest ID: ${isGuestLogIn ? int.parse(Get.find<AuthController>().getGuestId()) : 0}");
                  // print("Is Buy Now: ${widget.fromCart ? 0 : 1}");
                  // print("Guest Email: ${isGuestLogIn ? finalAddress.email : null}");
