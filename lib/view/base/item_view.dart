...
class _ItemsViewState extends State<ItemsView> {
  ...
  Widget build(BuildContext context) {
    ...
    return !isNull ? length > 0 ? GridView.builder(
      key: UniqueKey(),
...
class ItemsViewSections extends StatefulWidget {
  final List<Item?>? items;
  final List<Store?>? stores;
  final bool isStore;
  final EdgeInsetsGeometry padding;
  ...
  final bool isFeatured;
  final bool? isFoodOrGrocery;
  const ItemsViewSections({Key? key, required this.stores, required this.items, required this.isStore, this.isScrollable = false,
    this.shimmerLength = 20, this.padding = const EdgeInsets.all(Dimensions.paddingSizeSmall), this.noDataText,
    this.isCampaign = false, this.inStorePage = false, this.isFeatured = false,
    this.isFoodOrGrocery = true}) : super(key: key);

  @override
  State<ItemsViewSections> createState() => _ItemsViewStateSections();
}

class _ItemsViewStateSections extends State<ItemsViewSections> {
  @override
  Widget build(BuildContext context) {
    bool isNull = true;
    int length = 0;
    ...
    return !isNull ? GridView.builder(
      key: UniqueKey(),
      gridDelegate: SliverGridDelegateWithFixedCrossAxisCount(
