                ...
                Column(mainAxisAlignment: MainAxisAlignment.start, children: [
                  Expanded(
                    ...
                    child: Stack(children: [
                      ...
                      Get.find<ItemController>().isAvailable(item!) ? const SizedBox() : NotAvailableWidget(radius: Dimensions.radiusLarge, ),
...
class ItemWidgetSections extends StatelessWidget {
  final Item? item;
  final Store? store;
  final bool isStore;
  final int index;
  ...
  final double? imageWidth;
  final bool? isCornerTag;
  const ItemWidgetSections({Key? key, required this.item, required this.isStore, required this.store, required this.index,
    required this.length, this.inStore = false, this.isCampaign = false, this.isFeatured = false,
    this.fromCartSuggestion = false, this.imageHeight, this.imageWidth, this.isCornerTag = false}) : super(key: key);
    ...
    return InkWell(
      onTap: () {
        if(isStore) {
          ...
          Get.find<ItemController>().navigateToItemPageSections(item, context, inStore: inStore, isCampaign: isCampaign);
        }
      },
      child: Container(
        width: 200,
